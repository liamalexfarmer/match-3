--[[
	...
	Represents the beginning of the game--how it looks on startup.
	It shows the title of the game, and a "Press Enter to Play" message.
...
function StartState:init(  )
    ...
    --the letters for the title, and their spacing relative to center
    self.letterTable = {
    	{'M', -108},
        {'A', -64},
        {'T', -28},
    ...
    self.colorTimer = Timer.every(0.075, function()
    	...
    	--enables looping from end to start for function below
    ...
    --create a playset of tiles for display only using previously defined positions table
    for i = 1, 64 do
    ...
    --when an option gets selected, pause input
	...
	if not self.pauseInput then
		...
		if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
			if self.currentMenuItem == 1 then
				...
				--create tween using timer, that turns the screen white and transitions when the animation ends
				Timer.tween(1, {
			...
			--turn off input during transitions
...
function StartState:render(  )
	--render all tiles and shadows
	for y = 1, 8 do
		for x = 1, 8 do

			--render shadow before tiles
	...
	--overlay some shadow over the background/tiles
	...
	--transitional rectangle; covers the screen in white
...
function StartState:drawMatch3Text( y )
	...
	--50% opacity rectangle behind the MATCH 3 text
	...
	--draw Match 3 letters in their cycling colors
	--refers to the letter table for letter string and x offset
	for i = 1, 6 do
...
function StartState:drawOptions( y )
	...
	-- rectangle behind the options menu
	love.graphics.setColor(1, 1, 1, 128/255)
	...
	if self.currentMenuItem == 1 then
		...
		love.graphics.setColor(48/255, 96/266, 130/255, 1)
	end
...
--[[
	Elaborative shadow drawing helper function.
	Uses multiple copies at different opacity intervals to create contour shading
]]
